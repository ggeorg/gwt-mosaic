...
public class WidgetLocation extends AbstractLocation {
   ...
   * of <code>widget</code>. Therefore coordinates returned may be negative
   * or may exceed the dimensions of <code>reference</code>.
   * 
  ...
  private void internalSetReference(Widget reference) {
    ...
    if (reference == null || reference == RootPanel.get()) {
      ...
      referenceAdjustLeft = reference.getAbsoluteLeft() + border[3];
      referenceAdjustTop = reference.getAbsoluteTop() + border[0];
    }
  }
