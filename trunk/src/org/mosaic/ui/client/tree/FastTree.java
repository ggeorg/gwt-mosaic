...
public class FastTree extends Panel implements HasWidgets, HasFocus, HasFastTreeItems {
  ...
  private final String STYLENAME_DEFAULT = "gwt-FastTree";

  private final String STYLENAME_SELECTION = "selection-bar";

  private final FocusImpl impl = FocusImpl.getFocusImplForPanel();
  ...
  public void onBrowserEvent(Event event) {
    ...
    switch (eventType) {
      ...
      case Event.ONKEYPRESS: {
        ...
        if (DOM.isArrowKey(DOM.eventGetKeyCode(event))) {
  ...
  protected void keyboardNavigation(Event event) {
    ...
    if (curSelection == null) {
      ...
      switch (DOM.standardizeKeycode(DOM.eventGetKeyCode(event))) {
