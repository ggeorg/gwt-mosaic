...
public class Window extends Container {
	 ...
	 */
	public interface Skin extends gwt.mosaic.client.wtk.Skin {
		...
		public void setKeyStroke(Keyboard.KeyStroke keyStroke) {
			...
			if (keyStroke != previousKeyStroke) {
				if (window != null) {
					...
					window.windowActionMappingListeners.keyStrokeChanged(this,
							previousKeyStroke);
				}
		...
		public void setAction(Action action) {
			...
			if (action != previousAction) {
				if (window != null) {
					...
					window.windowActionMappingListeners.actionChanged(this,
							previousAction);
				}
	...
	public class ActionMappingSequence implements Sequence<ActionMapping> {
		...
		public int add(ActionMapping actionMapping) {
			if (actionMapping.window != null) {
				throw new IllegalArgumentException(
						"Action mapping already has a window.");
			}
			...
			if (actionMap.containsKey(actionMapping.keyStroke)) {
				throw new IllegalArgumentException("A mapping for "
						+ actionMapping.keyStroke + " already exists.");
			}
		...
		public Sequence<ActionMapping> remove(int index, int count) {
			Sequence<ActionMapping> removed = actionMappings.remove(index,
					count);
			...
			windowActionMappingListeners.actionMappingsRemoved(Window.this,
					index, removed);
...
//	}
	...
	private static class WindowListenerList extends
			ListenerList<WindowListener> implements WindowListener {
		@Override
...
//		}
	...
	private static class WindowStateListenerList extends
			ListenerList<WindowStateListener> implements WindowStateListener {
		@Override
	...
	private static class WindowActionMappingListenerList extends
		...
		@Override
		public void actionChanged(Window.ActionMapping actionMapping,
	...
	private static class WindowClassListenerList extends
			ListenerList<WindowClassListener> implements WindowClassListener {
		@Override
	...
	protected void setParent(Container parent) {
		if (parent != null && (!(parent instanceof Display))) {
			throw new IllegalArgumentException(
					"Window parent must be null or display, cannot be "
							+ parent);
		}
	 ...
	 * 
	 * @param owner
	 *            The window's owner. The window is opened on the owner's
	 *            display.
	 */
	 ...
	 * 
	 * @param owner
	 *            The window's owner, or
	 *            <tt>null<tt> if the window has no owner.
	 */
	public void open(Display display, Window owner) {
		...
		if (owner != null) {
			...
			if (isOwner(owner)) {
				throw new IllegalArgumentException(
						"owner is an owned descendant of this window.");
			}
		}
		...
		if (isOpen()) {
			if (getDisplay() != display) {
				throw new IllegalStateException(
						"Window is already open on a different display.");
			}

			if (this.owner != owner) {
				throw new IllegalStateException(
						"Window is already open with a different owner.");
			}
		}
...
//	}
	 ...
	 * <p>
	 * If the icon already exists in the application context resource cache, the
	 * cached value will be used. Otherwise, the icon will be loaded
...
//	}
	 ...
	 * 
	 * @param activeWindow
	 *            The window to activate, or <tt>null</tt> to clear the active
	 *            window.
	 */
	...
	public void moveToFront() {
		...
		if (ownedWindowCount == 0) {
			...
			// preserving the
			// current z-order
			ArrayList<Integer> ownedWindowIndexes = new ArrayList<Integer>(
					ownedWindowCount);
			...
			ArrayList<Window> sortedOwnedWindows = new ArrayList<Window>(
					ownedWindows.getLength());
	...
	public void align(Bounds bounds, HorizontalAlignment horizontalAlignment,
			int horizontalOffset, VerticalAlignment verticalAlignment,
			int verticalOffset) {
		...
		if (horizontalAlignment == HorizontalAlignment.LEFT) {
			...
			x = bounds.x
					+ (int) Math
							.round((double) (bounds.width - size.width) / 2);
		} else {
			throw new IllegalArgumentException(
		...
		if (verticalAlignment == VerticalAlignment.TOP) {
			...
			y = bounds.y
					+ (int) Math
							.round((double) (bounds.height - size.height) / 2);
		} else {
			throw new IllegalArgumentException(
