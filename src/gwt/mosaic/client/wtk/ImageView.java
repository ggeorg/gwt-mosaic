...
public class ImageView extends Component {
	...
	public interface ImageBindMapping {
		 ...
		 * @param value
		 */
		public String toImageURL(Object value);
	...
	private static class ImageViewListenerList extends
			ListenerList<ImageViewListener> implements ImageViewListener {
		@Override
	...
	private static class ImageViewBindingListenerList extends
			ListenerList<ImageViewBindingListener> implements
		...
		public void imageBindMappingChanged(ImageView imageView,
			...
			for (ImageViewBindingListener listener : this) {
				listener.imageBindMappingChanged(imageView,
	...
	//private static HashMap<java.net.URI, ArrayList<ImageView>> loadMap = new HashMap<java.net.URI, ArrayList<ImageView>>();
	...
	// }
	//
	// Image image = (Image) ApplicationContext.getResourceCache().get(
	// imageURL);
	//
	// if (image == null) {
	...
	// } else {
	// Image.load(imageURL, new TaskAdapter<Image>(
	// new TaskListener<Image>() {
	// @Override
	// public void taskExecuted(Task<Image> task) {
	// Image image = task.getResult();
	//
	// // Update the contents of all image views
	// // that requested this
	// // image
	// for (ImageView imageView : loadMap
	// .get(imageURI)) {
	 ...
	 * 
	 * @param asynchronous
	 *            <tt>true</tt> if images specified via URL will be loaded in
	 *            the background; <tt>false</tt> if they will be loaded
	 *            synchronously.
	 */
	...
	public void setImageBindType(BindType imageBindType) {
		...
		if (previousImageBindType != imageBindType) {
			...
			imageViewBindingListeners.imageBindTypeChanged(this,
	...
	public void setImageBindMapping(ImageBindMapping imageBindMapping) {
		...
		if (previousImageBindMapping != imageBindMapping) {
			...
			imageViewBindingListeners.imageBindMappingChanged(this,
					previousImageBindMapping);
		}
	}
