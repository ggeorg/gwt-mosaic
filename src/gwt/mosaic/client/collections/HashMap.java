...
import gwt.mosaic.client.util.EmptyIterator;
...
public class HashMap<K, V> implements Map<K, V>, Serializable {
	...
	private class KeyIterator implements Iterator<K> {
		...
		private Iterator<Pair<K, V>> getBucketIterator(int bucketIndex) {
			...
			return (bucket == null) ? new EmptyIterator<Pair<K, V>>() : bucket
					.iterator();
		}
	}
	...
	public HashMap(Map<K, V> map) {
		this(Math.max((int) (map.getCount() / DEFAULT_LOAD_FACTOR) + 1,
				DEFAULT_CAPACITY));
	...
	public Iterator<K> iterator() {
		return (keys == null) ? new KeyIterator() : new ImmutableIterator<K>(
				keys.iterator());
	}
