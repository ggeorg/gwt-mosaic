 ...
 * @author ggeorg
 */
public class ListStoreProvider<T> implements Provider<T> {

	private final List<T> store;
	private final DataFactory<T> factory;

	public ListStoreProvider(List<T> store, DataFactory<T> factory) {
		this.store = store;
	...
	public void provideData(DataSet<T> dataSet, List<T> data,
		...
		data.clear();
		data.addAll(store);
		callback.onSuccess(new Range(0, data.size()));
	}
	...
	public void createNew(DataSet<T> dataSet, AsyncCallback<T> asyncCallback) {
		asyncCallback.onSuccess(factory.create());
	}
