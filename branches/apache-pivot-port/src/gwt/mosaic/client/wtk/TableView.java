...
import gwt.mosaic.client.wtk.content.TableViewCellRenderer;
...
public class TableView extends Component {
	...
	public static final class Column {
		 ...
		 * 
		 * @param relative
		 *            If <tt>true</tt>, specifies a relative column width;
		 ...
		 * 
		 * @param headerData
		 *            The column header data, or <tt>null</tt> for no header
		 *            data.
		 */
		public void setHeaderData(Object headerData) {
			...
			if (previousHeaderData != headerData) {
				...
				if (tableView != null) {
					tableView.tableViewColumnListeners.columnHeaderDataChanged(
							this, previousHeaderData);
				}
			}
		}
		...
		public void setHeaderDataRenderer(HeaderDataRenderer headerDataRenderer) {
			...
			if (previousHeaderDataRenderer != headerDataRenderer) {
				...
				if (tableView != null) {
					tableView.tableViewColumnListeners
							.columnHeaderDataRendererChanged(this,
		 ...
		 *            '*' character, it is treated as a relative value.
		 *            Otherwise, it is considered an absolute value.
		 */
		 ...
		 * 
		 * @param relative
		 *            <tt>true</tt> if the column width is relative,
		...
		public void setWidthLimits(int minimumWidth, int maximumWidth) {
			...
			if (maximumWidth < minimumWidth) {
				throw new IllegalArgumentException(
						"Maximum width is smaller than minimum width.");
			}
		...
		public void setFilter(Object filter) {
			...
			if (previousFilter != filter) {
				...
				if (tableView != null) {
					tableView.tableViewColumnListeners.columnFilterChanged(
							this, previousFilter);
				}
			}
		}
		 ...
		 * 
		 * @param cellRenderer
		 *            The cell renderer that is used to draw the contents of
		 *            this column.
		 */
		public void setCellRenderer(CellRenderer cellRenderer) {
			...
			if (previousCellRenderer != cellRenderer) {
				...
				if (tableView != null) {
					tableView.tableViewColumnListeners
							.columnCellRendererChanged(this,
	...
	public interface CellRenderer extends Renderer {
		 ...
		 * 
		 * @param row
		 *            The row to render, or <tt>null</tt> if called to calculate
		 *            preferred height for skins that assume a fixed renderer
		 *            height.
		 * 
		 * @param rowIndex
		 *            The index of the row being rendered, or <tt>-1</tt> if
		 ...
		 * @param columnName
		 * 
		 * @return The cell data's string representation, or <tt>null</tt> if
		 *         the data does not have a string representation.
		 *         <p>
	...
	public interface HeaderDataRenderer extends Renderer {
		 ...
		 */
		public void render(Object data, int columnIndex,
				TableViewHeader tableViewHeader, String columnName,
				boolean highlighted);
	...
	public interface RowEditor {
		 ...
		 * 
		 * @param result
		 *            <tt>true</tt> to perform the edit; <tt>false</tt> to
		 *            cancel it.
		 */
	...
	public interface SelectedRowBindMapping {
		 ...
		 *            The value to locate.
		 * 
		 * @return The index of first occurrence of the value if it exists in
		 *         the list; <tt>-1</tt>, otherwise.
		 */
	...
	public final class ColumnSequence implements Sequence<Column>,
		...
		public Sequence<Column> remove(int index, int count) {
			...
			if (count > 0) {
				...
				tableViewColumnListeners.columnsRemoved(TableView.this, index,
						removed);
			}
	...
	public final class SortDictionary implements
			Dictionary<String, SortDirection>, Iterable<String> {
		...
		public SortDirection put(String columnName, SortDirection sortDirection) {
			...
			if (sortDirection == null) {
				...
				if (update) {
					tableViewSortListeners.sortUpdated(TableView.this,
							columnName, previousSortDirection);
				} else {
					sortList.add(columnName);
					tableViewSortListeners
							.sortAdded(TableView.this, columnName);
				}
			}
		...
		public SortDirection remove(String columnName) {
			...
			if (containsKey(columnName)) {
				...
				tableViewSortListeners.sortRemoved(TableView.this, columnName,
						sortDirection);
			}
		...
		public Dictionary.Pair<String, SortDirection> get(int index) {
			...
			return new Dictionary.Pair<String, SortDirection>(columnName,
					sortDirection);
		}
	...
	private static class TableViewListenerList extends
			ListenerList<TableViewListener> implements TableViewListener {
		@Override
		...
		public void disabledRowFilterChanged(TableView tableView,
			...
			for (TableViewListener listener : this) {
				listener.disabledRowFilterChanged(tableView,
	...
	private static class TableViewColumnListenerList extends
			ListenerList<TableViewColumnListener> implements
		...
		@Override
		public void columnHeaderDataChanged(Column column,
		...
		public void columnHeaderDataRendererChanged(TableView.Column column,
			...
			for (TableViewColumnListener listener : this) {
				listener.columnHeaderDataRendererChanged(column,
						previousHeaderDataRenderer);
			}
		}

		@Override
		public void columnWidthChanged(Column column, int previousWidth,
		...
		@Override
		public void columnWidthLimitsChanged(Column column,
				int previousMinimumWidth, int previousMaximumWidth) {
			for (TableViewColumnListener listener : this) {
				listener.columnWidthLimitsChanged(column, previousMinimumWidth,
		...
		@Override
		public void columnCellRendererChanged(Column column,
	...
	private static class TableViewRowListenerList extends
			ListenerList<TableViewRowListener> implements TableViewRowListener {
		@Override
	...
	private static class TableViewSelectionListenerList extends
			ListenerList<TableViewSelectionListener> implements
		...
		public void selectedRangesChanged(TableView tableView,
				Sequence<Span> previousSelection) {
			for (TableViewSelectionListener listener : this) {
	...
	private static class TableViewSortListenerList extends
			ListenerList<TableViewSortListener> implements
		...
		public void sortUpdated(TableView tableView, String columnName,
				SortDirection previousSortDirection) {
			for (TableViewSortListener listener : this) {
				listener.sortUpdated(tableView, columnName,
	...
	private static class TableViewBindingListenerList extends
			ListenerList<TableViewBindingListener> implements
		...
		public void tableDataBindTypeChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.tableDataBindTypeChanged(tableView,
		...
		public void tableDataBindMappingChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.tableDataBindMappingChanged(tableView,
		...
		public void selectedRowKeyChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.selectedRowKeyChanged(tableView,
		...
		public void selectedRowBindTypeChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.selectedRowBindTypeChanged(tableView,
		...
		public void selectedRowBindMappingChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.selectedRowBindMappingChanged(tableView,
		...
		public void selectedRowsKeyChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.selectedRowsKeyChanged(tableView,
		...
		public void selectedRowsBindTypeChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.selectedRowsBindTypeChanged(tableView,
		...
		public void selectedRowsBindMappingChanged(TableView tableView,
			...
			for (TableViewBindingListener listener : this) {
				listener.selectedRowsBindMappingChanged(tableView,
	...
	private ListListener<Object> tableDataListener = new ListListener<Object>() {
		...
		public void itemInserted(List<Object> list, int index) {
			...
			if (updated > 0) {
				tableViewSelectionListeners.selectedRangesChanged(
						TableView.this, getSelectedRanges());
			}
		}

		@Override
		public void itemsRemoved(List<Object> list, int index,
				Sequence<Object> items) {
			...
			if (updated > 0) {
				...
				if (selectMode == SelectMode.SINGLE
					...
					tableViewSelectionListeners.selectedRowChanged(
							TableView.this, null);
				}
			}
		}
		...
		@Override
		public void itemUpdated(List<Object> list, int index,
				Object previousItem) {
	...
	protected void setSkin(BeanAdapter<? extends gwt.mosaic.client.wtk.Skin> styles) {
		if (!(styles.getBean() instanceof TableView.Skin)) {
	...
	public void setTableData(List<?> tableData) {
		...
		if (previousTableData != tableData) {
			...
			if (previousTableData != null) {
				...
				((List<Object>) previousTableData).getListListeners().remove(
						tableDataListener);
			} else {
				cleared = 0;
			}

			((List<Object>) tableData).getListListeners()
					.add(tableDataListener);
	 ...
	 * 
	 * @param tableData
	 *            A JSON string (must begin with <tt>[</tt> and end with
	 *            <tt>]</tt>) denoting the data to be presented by the table
	 *            view.
	 */
	 ...
	 * 
	 * @param tableData
	 *            A URL referring to a JSON file containing the data to be
...
//
//		try {
//			setTableData((List<?>) jsonSerializer.readObject(tableData
//					.openStream()));
//		} catch (SerializationException exception) {
//			throw new IllegalArgumentException(exception);
...
//	}
	...
	public int getSelectedIndex() {
		if (selectMode != SelectMode.SINGLE) {
			throw new IllegalStateException(
					"Table view is not in single-select mode.");
		}

		return (rangeSelection.getLength() == 0) ? -1
				: rangeSelection.get(0).start;
	}
	 ...
	 *         that the returned list is a wrapper around the actual selection,
	 *         not a copy. Any changes made to the selection state will be
	 *         reflected in the list, but events will not be fired.
	 */
	...
	public Sequence<Span> setSelectedRanges(Sequence<Span> selectedRanges) {
		...
		Sequence<Span> previousSelectedRanges = this.rangeSelection
				.getSelectedRanges();
		Object previousSelectedRow = (selectMode == SelectMode.SINGLE) ? getSelectedRow()
				: null;
		...
		if (selectMode == SelectMode.SINGLE) {
			tableViewSelectionListeners.selectedRowChanged(this,
					previousSelectedRow);
		}
	...
	public int getFirstSelectedIndex() {
		return (rangeSelection.getLength() > 0) ? rangeSelection.get(0).start
				: -1;
	}
	...
	public int getLastSelectedIndex() {
		return (rangeSelection.getLength() > 0) ? rangeSelection
				.get(rangeSelection.getLength() - 1).end : -1;
	}
	...
	public Sequence<Span> addSelectedRange(int start, int end) {
		if (selectMode != SelectMode.MULTI) {
			throw new IllegalStateException(
					"Table view is not in multi-select mode.");
		}
		...
		for (int i = 0; i < n; i++) {
			...
			tableViewSelectionListeners.selectedRangeAdded(this,
					addedRange.start, addedRange.end);
		}
	...
	public Sequence<Span> removeSelectedRange(int start, int end) {
		...
		for (int i = 0; i < n; i++) {
			...
			tableViewSelectionListeners.selectedRangeRemoved(this,
					removedRange.start, removedRange.end);
		}
	...
	public void setSelectedRow(Object row) {
		setSelectedIndex((row == null) ? -1 : ((List<Object>) tableData)
				.indexOf(row));
	}
	...
	public Dictionary<String, SortDirection> setSort(String columnName,
			SortDirection sortDirection) {
		Dictionary.Pair<String, SortDirection> sort = new Dictionary.Pair<String, SortDirection>(
	 ...
	 * 
	 * @param sort
	 *            A sequence of key/value pairs representing the sort. Keys
	...
	@SuppressWarnings("unchecked")
	private Sequence<Dictionary.Pair<String, SortDirection>> parseSort(
			String json) throws SerializationException {
//		ArrayList<Dictionary.Pair<String, SortDirection>> sort = new ArrayList<Dictionary.Pair<String, SortDirection>>();
//
//		List<?> list = JSONSerializer.parseList(json);
//		for (Object item : list) {
//			Map<String, ?> map = (Map<String, ?>) item;
//
//			Dictionary.Pair<String, SortDirection> pair = new Dictionary.Pair<String, SortDirection>(
//					(String) map.get(COLUMN_NAME_KEY),
//					SortDirection.valueOf(((String) map.get(SORT_DIRECTION_KEY))
...
//		return sort;
	 ...
	 *            The disabled row filter, or <tt>null</tt> for no disabled row
	 *            filter.
	 */
	public void setDisabledRowFilter(Filter<?> disabledRowFilter) {
		...
		if (previousDisabledRowFilter != disabledRowFilter) {
			...
			tableViewListeners.disabledRowFilterChanged(this,
					previousDisabledRowFilter);
		}
	}
	...
	public void setTableDataKey(String tableDataKey) {
		...
		if (previousTableDataKey != tableDataKey) {
			...
			tableViewBindingListeners.tableDataKeyChanged(this,
	...
	public void setTableDataBindType(BindType tableDataBindType) {
		...
		if (previousTableDataBindType != tableDataBindType) {
			...
			tableViewBindingListeners.tableDataBindTypeChanged(this,
					previousTableDataBindType);
		}
	}
	...
	public void setTableDataBindMapping(
			TableDataBindMapping tableDataBindMapping) {
	...
	public void setSelectedRowKey(String selectedRowKey) {
		...
		if (previousSelectedRowKey != selectedRowKey) {
			...
			tableViewBindingListeners.selectedRowKeyChanged(this,
					previousSelectedRowKey);
		}
	}
	...
	public void setSelectedRowBindType(BindType selectedRowBindType) {
		...
		if (previousSelectedRowBindType != selectedRowBindType) {
			...
			tableViewBindingListeners.selectedRowBindTypeChanged(this,
					previousSelectedRowBindType);
		}
	}
	...
	public void setSelectedRowBindMapping(
			SelectedRowBindMapping selectedRowBindMapping) {
	...
	public void setSelectedRowsKey(String selectedRowsKey) {
		...
		if (previousSelectedRowsKey != selectedRowsKey) {
			...
			tableViewBindingListeners.selectedRowsKeyChanged(this,
					previousSelectedRowsKey);
		}
	}
	...
	public void setSelectedRowsBindMapping(
			SelectedRowBindMapping selectedRowsBindMapping) {
	...
	public void load(Object context) {
		...
		case MULTI: {
			...
			if (selectedRowsKey != null
				...
				Sequence<Object> rows = (Sequence<Object>) JSON.get(context,
						selectedRowsKey);
			...
			if (selectedRowKey != null && selectedRowBindType != BindType.LOAD) {
				...
				if (selectedIndex == -1) {
					...
					if (selectedRowBindMapping == null) {
						...
						row = selectedRowBindMapping.get(tableData,
								selectedIndex);
					}
				}
