...
import gwt.mosaic.client.collections.LinkedList;
...
public class TextInput extends Component {
	...
	private static class TextInputListenerList extends
			ListenerList<TextInputListener> implements TextInputListener {
		@Override
	...
	private static class TextInputContentListenerList extends
			ListenerList<TextInputContentListener> implements
		...
		public Vote previewInsertText(TextInput textInput, CharSequence text,
			...
			for (TextInputContentListener listener : this) {
				vote = vote.tally(listener.previewInsertText(textInput, text,
						index));
			}
		...
		public Vote previewRemoveText(TextInput textInput, int index, int count) {
			...
			for (TextInputContentListener listener : this) {
				vote = vote.tally(listener.previewRemoveText(textInput, index,
						count));
			}
	...
	private static class TextInputSelectionListenerList extends
			ListenerList<TextInputSelectionListener> implements
	...
	private static class TextInputBindingListenerList extends
			ListenerList<TextInputBindingListener> implements
		...
		public void textBindMappingChanged(TextInput textInput,
			...
			for (TextInputBindingListener listener : this) {
				listener.textBindMappingChanged(textInput,
	...
	protected void setSkin(
		...
		if (!(styles.getBean() instanceof TextInput.Skin)) {
	...
	private void insertText(CharSequence text, int index,
		...
		if (characters.length() + text.length() > maximumLength) {
			throw new IllegalArgumentException(
					"Insertion of text would exceed maximum length.");
		}

		if (text.length() > 0) {
			Vote vote = textInputContentListeners.previewInsertText(this, text,
					index);
			...
			if (vote == Vote.APPROVE) {
				...
				// Update the valid flag
				boolean previousTextValid = textValid;
				textValid = (validator == null) ? true : validator
						.isValid(getText());

				// Fire change events
				textInputContentListeners.textInserted(this,
						previousSelectionStart, text.length());
				textInputContentListeners.textChanged(this);
	...
	private void removeText(int index, int count, boolean addToEditHistory) {
		if (count > 0) {
			Vote vote = textInputContentListeners.previewRemoveText(this,
					index, count);
			...
			if (vote == Vote.APPROVE) {
				...
				// Fire change events
				textInputContentListeners.textRemoved(this, selectionStart,
						count);
				textInputContentListeners.textChanged(this);
	 ...
	 *         points are inclusive. Returns <tt>null</tt> if the selection
	 *         length is <tt>0</tt>.
	 */
	...
	public final void setSelection(Span selection) {
		...
		setSelection(Math.min(selection.start, selection.end),
				(int) selection.getLength());
	}
	...
	public void setMaximumLength(int maximumLength) {
		...
		if (previousMaximumLength != maximumLength) {
			...
			if (length > maximumLength) {
				...
				textInputContentListeners.textRemoved(this, maximumLength,
						count);
				textInputContentListeners.textChanged(this);
			}

			textInputListeners
					.maximumLengthChanged(this, previousMaximumLength);
		}
	}
